  ...
  Future<Map<String, dynamic>> getAggregatedHealthDataForModel({
    ...
    for (int i = 0; i < daysWindow; i++) {
      ...
      try {
        ...
        // getSleepStatistics devuelve 0 para minutos si no hay datos o si realmente fue 0.
        // Contamos el día para el promedio de sueño si se procesó.
        totalSleepMinutesSum +=
            (sleepStats['totalSleepMinutes'] as int).toDouble();
        deepSleepMinutesSum +=
            (sleepStats['deepSleepMinutes'] as int).toDouble();
        lightSleepMinutesSum +=
            (sleepStats['lightSleepMinutes'] as int).toDouble();
        remSleepMinutesSum += (sleepStats['remSleepMinutes'] as int).toDouble();
        daysWithSleepData++;
      } catch (e) {
      ...
      try {
        ...
        // getStepsStatistics devuelve {'totalSteps': 0} si hay error o no hay datos (o son 0).
        // Contamos el día para el promedio de pasos.
